...
class Tenant(Base):
    ...
    primary_domain: Mapped[str | None] = mapped_column(String(256), nullable=True, unique=True, index=True)
    ...
    require_domain_match: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False, comment="Require email domain match for new users"
    )

    allow_any_github_user: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, comment="Allow any GitHub user (dev/testing mode)"
    )
    ...
    settings: Mapped[dict[str, Any] | None] = mapped_column(
        JSONType(), nullable=True, default=None, comment="Tenant-specific configuration"
    )

    # Tenant status
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False, comment="Whether tenant is active (can be disabled for suspension)"
    )
    ...
    applications: Mapped[list["Application"]] = relationship("Application", back_populates="tenant", lazy="dynamic")
    contexts: Mapped[list["Context"]] = relationship("Context", back_populates="tenant", lazy="dynamic")
    policies: Mapped[list["Policy"]] = relationship("Policy", back_populates="tenant", lazy="dynamic")
    teams: Mapped[list["Team"]] = relationship("Team", back_populates="tenant", lazy="dynamic")
