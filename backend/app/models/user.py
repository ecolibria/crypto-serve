...
class User(Base):
    ...
    custom_permissions: Mapped[list | None] = mapped_column(
        JSON, nullable=True, default=None, doc="Additional permissions beyond role defaults"
    )
    denied_permissions: Mapped[list | None] = mapped_column(
        JSON, nullable=True, default=None, doc="Permissions explicitly denied (overrides role)"
    )

    # Provisioning tracking
    provisioning_source: Mapped[str | None] = mapped_column(
        String(50), nullable=True, doc="How user was provisioned: first_user, domain, github_org, invitation, admin"
    )
    invitation_id: Mapped[str | None] = mapped_column(GUID(), ForeignKey("user_invitations.id"), nullable=True)

    # Relationships
    tenant: Mapped["Tenant"] = relationship("Tenant", back_populates="users", lazy="selectin")
    identities: Mapped[list["Identity"]] = relationship("Identity", back_populates="user", lazy="selectin")
    applications: Mapped[list["Application"]] = relationship("Application", back_populates="user", lazy="selectin")
    teams: Mapped[list["Team"]] = relationship("Team", secondary="user_teams", back_populates="users", lazy="selectin")
