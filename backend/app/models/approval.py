...
class ExpeditedApprovalRequest(Base):
    ...
    application_id: Mapped[str] = mapped_column(GUID(), ForeignKey("applications.id"), nullable=False, index=True)
    ...
    # Requester info
    requester_user_id: Mapped[str] = mapped_column(GUID(), ForeignKey("users.id"), nullable=False, index=True)
    ...
    # Approval status
    status: Mapped[str] = mapped_column(String(20), default=ApprovalStatus.PENDING.value, nullable=False, index=True)

    # Approver info (filled when approved/rejected)
    approved_by_user_id: Mapped[str | None] = mapped_column(GUID(), ForeignKey("users.id"), nullable=True)
