...
async def get_usage_stats(
    ...
    start_date: datetime | None = Query(default=None, description="Start of period (defaults to 30 days ago)"),
    end_date: datetime | None = Query(default=None, description="End of period (defaults to now)"),
    context_ids: list[str] | None = Query(default=None, description="Filter by specific contexts"),
):
    ...
    # Get total calls
    total_result = await db.execute(select(func.count(AuditLog.id)).where(and_(*base_conditions)))
    ...
    if context_rows:
        ...
        contexts_result = await db.execute(
            select(Context.name, Context.display_name).where(
                Context.name.in_(context_ids_list), Context.tenant_id == user.tenant_id
            )
        )
    ...
    error_query = (
        ...
        .group_by(AuditLog.context, AuditLog.error_message)
...
async def get_context_usage(
    ...
    context_result = await db.execute(
        select(Context).where(Context.name == context_name, Context.tenant_id == user.tenant_id)
    )
    context = context_result.scalar_one_or_none()

    if not context:
        raise HTTPException(status_code=404, detail=f"Context not found: {context_name}")
