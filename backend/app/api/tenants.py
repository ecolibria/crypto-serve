...
async def get_current_tenant_stats(
    ...
    admin_users = await db.scalar(
        select(func.count(User.id)).where(User.tenant_id == admin.tenant_id, User.is_admin == True)  # noqa: E712
    )
    ...
    active_identities = await db.scalar(
        select(func.count(Identity.id)).where(
            Identity.tenant_id == admin.tenant_id, Identity.status == IdentityStatus.ACTIVE
        )
    )
...
async def update_current_tenant(
    ...
    if update.primary_domain is not None:
        ...
        existing = await db.execute(
            select(Tenant).where(Tenant.primary_domain == update.primary_domain, Tenant.id != tenant.id)
        )
...
async def list_tenant_users(
    ...
    for user in users:
        ...
        identity_count = await db.scalar(
            select(func.count(Identity.id)).where(Identity.user_id == user.id, Identity.tenant_id == admin.tenant_id)
        )
...
async def remove_tenant_user(
    ...
    identities_result = await db.execute(
        select(Identity).where(Identity.user_id == user.id, Identity.tenant_id == admin.tenant_id)
    )
...
async def create_tenant(
    ...
    if request.primary_domain:
        existing_domain = await db.execute(select(Tenant).where(Tenant.primary_domain == request.primary_domain))
...
async def get_tenant_stats(
    ...
    # User counts
    total_users = await db.scalar(select(func.count(User.id)).where(User.tenant_id == tenant_id))
    admin_users = await db.scalar(
        select(func.count(User.id)).where(User.tenant_id == tenant_id, User.is_admin == True)  # noqa: E712
    )

    # Identity counts
    total_identities = await db.scalar(select(func.count(Identity.id)).where(Identity.tenant_id == tenant_id))
    active_identities = await db.scalar(
        select(func.count(Identity.id)).where(Identity.tenant_id == tenant_id, Identity.status == IdentityStatus.ACTIVE)
    )

    # Context counts
    total_contexts = await db.scalar(select(func.count(Context.id)).where(Context.tenant_id == tenant_id))

    # Operation counts
    total_operations = await db.scalar(select(func.count(AuditLog.id)).where(AuditLog.tenant_id == tenant_id))
